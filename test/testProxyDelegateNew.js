...
contract("ProxyDelegateNew", (accounts) => {
  let proxy;
  let lib;
  ...
  before(async () => {
    ...
    proxy = await ProxyDelegateNew.deployed(lib.address, { from: owner });
  });
  ...
  it("getMsgSender by delegatecall should pass", () => {
    ...
    return proxy.sendTransaction({ from: accounts[1], data }).then((tx) => {
      truffleAssert.eventEmitted(tx, "LogResult", (ev) => {
        ...
        const regex = new RegExp(ProxyDelegateNew.address.slice(2), "i");
